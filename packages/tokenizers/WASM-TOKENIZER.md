...
- **Browsers / edge**: if `tiktoken` is unavailable, use **`@contextcraft/tokenizers`** fallbacks (`CharEstimatorTokenizer`, `gpt-tokenizer`, or provider-specific adapters) and/or supply a `tokenAccountant` on the core side for authoritative totals.
