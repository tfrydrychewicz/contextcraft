...
import { Context, ContextOrchestrator, createContext, toTokenCount } from 'contextcraft';
...
describe('otelPlugin integration', () => {
  it('emits build + overflow spans and records histograms on truncate overflow', async () => {
    ...
    expect(build?.attributes['contextcraft.total_budget']).toBeGreaterThan(0);
    expect(build?.attributes['contextcraft.message_count']).toBeGreaterThanOrEqual(0);
